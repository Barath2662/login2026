...
export const AdminIdRecordsView = () => {
  ...
  const fetchRecords = async () => {
    try {
      const { data } = await api.get('/admin/id-records');
      setRecords(data.data);
    } catch (err: any) {
      setError(err.response?.data?.error || 'Failed to fetch ID records');
    } finally {
      setLoading(false);
    }
  };
  ...
  const viewImages = async (record: IdRecord) => {
    ...
    try {
      if (record.idCardFrontUrl) {
        const res1 = await api.post('/storage/signed-url', { path: record.idCardFrontUrl });
        setFrontUrl(res1.data.signedUrl);
      }
      if (record.idCardBackUrl) {
        const res2 = await api.post('/storage/signed-url', { path: record.idCardBackUrl });
        setBackUrl(res2.data.signedUrl);
      }
    } catch (error) {
